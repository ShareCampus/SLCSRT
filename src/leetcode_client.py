...
class LeetcodeClient:
    ...
    def __init__(
        self, cookie, sleep_time=5, base_url="https://leetcode.cn/", logger=None
    ) -> None:
    ...
    def login(self) -> None:
        ...
        for try_cnt in range(ATTEMPT):
            ...
            result = self.client.post(login_url, headers=login_header)
    ...
    def downloadCode(self, submission) -> str:
        ...
        response = self.client.post(
            self.endpoint + self.GRAPHQL_PATH, json=data, headers=self.headers
        )
    ...
    def getSubmissionList(self, page_num):
        ...
        submissions_url = (
            "https://leetcode.cn/api/submissions/?offset={page_num}&limit=40".format(
                page_num=page_num
            )
        )
