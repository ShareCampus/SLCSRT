...
def generate_path_with_dir(problem_id, problem_title, submission_language, OUTPUT_DIR):
    # 如果题目是传统的数字题号
    if problem_id[0].isdigit():
        problem_id = int(problem_id)
        pathname = OUTPUT_DIR + "/" + "{:0=4}".format(problem_id) + "." + problem_title
        filename = (
            str(int("{:0=4}".format(problem_id)))
            + "-"
            + problem_title
...
def generate_path(problem_id, problem_title, submission_language, OUTPUT_DIR):
    # 如果题目是传统的数字题号
    if problem_id[0].isdigit():
        problem_id = int(problem_id)
        pathname = OUTPUT_DIR
